...
namespace Vfs.Zip
{
    public class ZipFileProvider : FileSystemProviderBase2//<ZipFileItem, ZipFolderItem>
    {
        ...
        /// 
        /// </summary>
        /// <param name="virFolderPath">相对root路径 必须以 '/'结尾</param>
        ...
        /// 
        /// </summary>
        /// <param name="virFilePath">相对root路径 不能以 '/结尾'</param>
        /// <param name="relativeRoot">只能是true</param>
        /// <returns></returns>
        public bool ExistFile(string virFilePath, bool relativeRoot) {
            string virFolderPath = virFilePath.Replace("\\", "/");
            if (virFolderPath[0] == '/') virFolderPath = virFolderPath.Remove(0, 1);
            //if (virFolderPath[virFolderPath.Length - 1] == '/') virFolderPath = virFolderPath.Remove(virFolderPath.Length - 1);
            FileInfo zipfilep = this.Configuration.ZipFileInfo;
            ZipFile zf = Ionic.Zip.ZipFile.Read(zipfilep.FullName);
            bool find = false;
            foreach (string name in zf.EntryFileNames) {
                if (string.Compare(virFolderPath, name, true) == 0) {
                    find = true;
                    break;
                }
