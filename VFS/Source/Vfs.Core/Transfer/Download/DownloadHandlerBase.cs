...
namespace Vfs.Transfer
{
        ...
        where TTransfer : DownloadTransfer<TFile>
    {
        ...
        protected DownloadHandlerBase(ITransferStore<TTransfer> transferStore)
            : base(transferStore) {
        }
        ...
        private Stream DownloadFileImpl(string transferId) {
            ...
            if (transfer.Token.TotalBlockCount == 0) {
                ...
                AuditHelper.AuditTransferOperation(Auditor, context, AuditEvent.FileDataDownloaded, transfer.TransferId, transfer.FileItem as IVirtualFileItem);
                return Stream.Null;
            }
