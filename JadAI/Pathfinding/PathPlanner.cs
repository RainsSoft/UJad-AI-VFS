...
using JadEngine.AI.Graphs;
using JadEngine.AI.Messaging;
using JadEngine.AI.Navigation;

#endregion

namespace JadEngine.AI.Pathfinding
{
	/// <summary>
	...
	/// owner should implement the <see cref="JadEngine.AI.Messaging.IMessageable" /> interface
	/// </remarks>
		...
		where T : WeightedEdge
	{
		...
		protected internal SearchState CycleOnce()
		{
			...
			if (result == SearchState.CompletedAndNotFound)
				MessageManagerRouter.Instance.SendMessage(0, owner.ID, 0, MessageType.PathNotAvailable, null);
		...
		private int ClosestNodeToPosition(Vector3 position)
		{
			...
			for (int i = 0; i < neighbours.Count; i++)
			{
				...
				{
					distance = (position - neighbours[i].Position).sqrMagnitude;
