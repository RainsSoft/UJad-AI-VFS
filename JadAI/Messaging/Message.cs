...
namespace JadEngine.AI.Messaging
{
	...
	public class Message
	{
		...
		/// The message type
		/// </summary>
		protected internal MessageType type;
		...
		public Message(int senderID, int recieverID, MessageType type, double dispatchTime, object extraInfo)
		{
			String message = String.Empty;

			if (ValidateID(senderID, ref message) == false)
				throw new JValidationException("senderID", this.GetType().ToString(), message);

			if (ValidateID(recieverID, ref message) == false)
				throw new JValidationException("recieverID", this.GetType().ToString(), message);

			if (ValidateTime(dispatchTime, ref message) == false)
				throw new JValidationException("dispatchTime", this.GetType().ToString(), message);
